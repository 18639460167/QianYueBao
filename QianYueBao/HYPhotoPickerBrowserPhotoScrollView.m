//
//  HYPhotoPickerBrowserPhotoScrollView.m
//  自定义tabbar
//
...
@interface HYPhotoPickerBrowserPhotoScrollView()<UIActionSheetDelegate,HYPhotoPickerBrowserPhotoViewDelegate>
{
    HYPhotoPickerBrowserPhotoView *_tapView; // for background taps
}
...
@end
@implementation HYPhotoPickerBrowserPhotoScrollView

- (id)init
{
    if (self = [super init])
    {
        _tapView = [[HYPhotoPickerBrowserPhotoView alloc]initWithFrame:self.bounds];
        ...
        // Image view
        _photoImageView = [[HYPhotoPickerBrowserPhotoImageView alloc] initWithFrame:CGRectZero];
        ...
        progressView.frame = CGRectMake(0, 0, HYPickerProgressViewW, HYPickerProgressViewH);
...
- (void)longGesture:(UILongPressGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan)
    {
        ...
        if (!self.sheet)
        {
            self.sheet = [[UIActionSheet alloc] initWithTitle:@"U-Travel" delegate:self cancelButtonTitle:LS(@"Cancel") destructiveButtonTitle:LS(@"Save_Photo") otherButtonTitles:nil, nil];
        }
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 0){
        ...
        else
        {
            ...
            {
                [self showMessageWithText:LS(@"Save_Fail")];
            }
        }
    }
}
...
- (void)setPhoto:(HYPhotoPickerBrowerPhoto *)photo
{
    _photo = photo;
    ...
    if (photo.photoURL.absoluteString.length)
    {
        ...
        {
            [[HYPhotoPickerDatas defaultPicker] getAssetsPhotoWithURLS:photo.photoURL callBack:^(id obj) {
        ...
        {
            UIImage *thumImage = photo.thumbImage;
            if (thumImage == nil)
            {
                thumImage = _photoImageView.image;
            }
            else
            {
                _photoImageView.image = thumImage;
            }
            _photoImageView.contentMode = UIViewContentModeScaleAspectFit;
            _photoImageView.frame = [HYPhotoRect setMaxMinzoomScalesForCurrentBoundWithIMageView:_photoImageView];
            ...
            // 请求图片
            [_photoImageView sd_setImageWithURL:photo.photoURL placeholderImage:thumImage options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
